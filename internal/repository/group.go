...
func (r *GroupRepo) GetGroups(ctx context.Context, userID string) ([]domain.Group, error) {
	rows, err := r.conn(ctx).QueryContext(ctx,
		"SELECT g.id, g.name, gm.is_default, g.created_at, g.modified_at FROM groups AS g INNER JOIN group_members gm ON g.id=gm.group_id WHERE gm.user_id = $1",
		userID,
	)
	if err != nil {
	...
	for rows.Next() {
		var g domain.Group
		err = rows.Scan(&g.ID, &g.Name, &g.IsDefault, &g.CreatedAt, &g.ModifiedAt)
		if err != nil {
			return nil, err
		}
